...
sequenceDiagram
    ...
    alt 서명 성공
        ...
        pg사 ->> 원큐 오더 서버: 39. 결제 결과 전송
        원큐 오더 서버 ->> 원큐 오더 서버: 40. 주문 상태 변경
        원큐 오더 서버 ->> 원큐 오더 웹: 41. 결과 전송
        원큐 오더 웹 ->> 사용자: 42. 결제 완료 화면 표시
        사용자 ->> 원큐 오더 웹: 43. 완료 버튼 클릭
        원큐 오더 웹 ->> 원큐 오더 서버: 44. 결제 완료 알림
        원큐 오더 서버 ->> pg사: 45. 결제 검증 요청 (paymentId)
        pg사 ->> pg사: 46. 상태 및 금액 검증
        pg사 ->> 원큐 오더 서버: 47. 검증 응답
        원큐 오더 서버 ->> 원큐 오더 웹: 48. 최종 확인 완료
    else 서명 실패
