...
//                .id(UUID.randomUUID().toString()) // jti 클레임: JWT 토큰 식별자를 지정
    ...
    public TokenDto refreshTokens(String refreshToken) {
        try {
            ...
            if (!userRole.equals(Role.MERCHANT) && !userRole.equals(Role.ADMIN)) {
                throw new JwtException("유효하지 않은 사용자 역할입니다.");
            } else if (userRole.equals(Role.MERCHANT) && !merchantRepository.existsById(userId)) {
                throw new JwtException("가맹점 정보를 찾을 수 없습니다.");
            } else if (userRole.equals(Role.ADMIN) && !adminRepository.existsById(userId)) {
                throw new JwtException("관리자 정보를 찾을 수 없습니다.");
            } else {
                log.info("사용자({})의 리프레시 토큰을 검증하는데 성공하였습니다.", userId);
            }

            // 새로운 토큰 발급
            TokenDto tokens = generateTokens(userId, userRole);
            log.info("사용자({})의 리프레시 토큰을 재발급하는데 성공하였습니다.", userId);
    ...
    public Role getUserRoleFromToken(String accessToken) {
        try {
            // 토큰 파싱
            Claims claims = Jwts.parser()
                    .verifyWith(secretKey)
            ...
            log.warn("유효하지 않은 토큰으로 사용자 역할을 추출하려고 시도했습니다.: {}", e.getMessage());
            throw new JwtException("토큰이 유효하지 않습니다.");
        }
    }
